...
/*
// add function for calculation of yearly expenses
// maybe mean yearly expenses
...
YearlyExpense yearlyExpenses[MAX_YEARS];
int yearCount = 0;

double meanYearlyExpenses() {
    double total = 0.0;
    int count = 0;
    char* char_date;
    ...
    for (int i = 0; i < expenseCount; i++) {
        char_date = strtok(expenses[i].date, "-");
        int year = atoi(char_date);
        double amount = expenses[i].amount;
        // const char* category = expenses[i].category;
        // double amount = expenses[i].amount;
        ...
        for (int j = 0; j < yearCount; j++) {
            if (strcmp(atoi(strtok(yearlyExpenses[j].date, '-')), year) == 0) {
                yearIndex = j;
                break;
            }
        }
        ...
        // Add the expense amount to the category total
        // categoryTotals[categoryIndex].total += amount;
        YearlyExpense[i].year = year;
        YearlyExpense[i].count = count;
        YearlyExpense[i].amount = total/YearlyExpense[i].count;
        count++;
    }
